...
// Content categories for 头条/公众号 articles
...
const showCategorySelector = computed(() => {
  return selectedType.value?.typeId === 'wechat_article' || selectedType.value?.typeId === 'toutiao_article'
})
...
function selectType(type: ProjectTypeDefinition) {
  ...
  // Set default project name based on type
  const now = new Date()
  const dateStr = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}`
  if (type.typeId === 'toutiao_article') {
    if (!name.value) name.value = `头条-${dateStr}`
  } else if (type.typeId === 'wechat_article') {
    if (!name.value) name.value = `公众号-${dateStr}`
  }
...
async function handleCreate() {
  ...
  try {
    ...
    if (showCategorySelector.value && contentCategory.value) {
      typeConfig.contentCategory = contentCategory.value
    }
      ...
      <div v-else class="modal-body">
        ...
        <div class="form-section">
          <div class="name-category-row">
            <div class="name-group" :class="{ 'has-category': showCategorySelector }">
            ...
            <div v-if="showCategorySelector" class="category-group">
              <label class="section-label">内容分类</label>
              <select v-model="contentCategory" class="category-select">
          ...
          <p v-if="showCategorySelector" class="helper-text">名称将作为文件夹名；AI 助手将根据分类提供更有针对性的写作建议</p>
          <p v-else class="helper-text">名称将作为文件夹名</p>
        </div>
...
.category-select:focus { border-color: var(--accent-color); }
