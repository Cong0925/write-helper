...
async function sendMessage(systemPrompt?: string, skillLabel?: string) {
  ...
  // Use skill system prompt if provided, otherwise default
  const systemMsg: ChatMessage = {
    role: 'system',
    content: systemPrompt || `你是一个专业的写作助手，帮助用户进行创作。当前项目类型：${appState.project?.projectType || '未知'}。请根据用户的指令提供高质量的中文写作辅助。`,
  }
